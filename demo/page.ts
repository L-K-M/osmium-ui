...
type DemoHandler = { postMessage(m: unknown): void };
const demoApp = (window as {
  webkit?: { messageHandlers?: { demo?: DemoHandler } };
}).webkit?.messageHandlers?.demo;
...
hosted = hostWindow(win, {
  title: spec.title,
  ...(spec.zoom ? { zoom: { standard: spec.standard ?? spec.size } } : {}),
